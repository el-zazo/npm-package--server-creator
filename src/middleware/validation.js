...
const schemas = {
  ...
  auth: {
    ...
    getRegisterSchema: (identifiantKey = "email", passwordKey = "password", additionalFields = {}) => {
      // Validate that all additionalFields values are Joi schema objects
      Object.entries(additionalFields).forEach(([key, value]) => {
        if (!value || typeof value.validate !== "function") {
          throw new Error(
            `additionalFields["${key}"] must be a Joi schema object. ` +
  ...
  crud: {
    ...
    getAll: Joi.object({
      ...
      no_pagination: Joi.string().default("false").valid("false", "true", "False", "True", "0", "1"),
    }),
