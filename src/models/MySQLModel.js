...
class MySQLModel {
  ...
  async getAll(options = {}) {
    const cacheKey = `${this.collectionName}::all::${JSON.stringify(options)}`;
  ...
  async getMany(query = {}, options = {}) {
    ...
    const executeQuery = async () => {
      try {
        ...
        // Normalize or/and filter keys to Sequelize Op.or/Op.and
        const normalizedQuery = normalizeMySQLFilter(query, Op);
    ...
    const cacheKey = `${this.collectionName}::query::${queryStr}_${JSON.stringify(options)}`;
  ...
  async addMany(dataArray) {
    try {
      const documents = await this.model.bulkCreate(dataArray, { returning: true });
      this.cache?.invalidateByPrefix(this.collectionName);
      return documents;
    } catch (error) {
      if (error.name === "SequelizeValidationError") {
  ...
  async updateManyByFilter(filter, data) {
    try {
      const queryFilter = normalizeMySQLFilter(filter, Op);
      ...
      this.cache?.invalidateByPrefix(this.collectionName);
      return { affectedCount: modifiedCount };
    } catch (error) {
      if (error.name === "SequelizeValidationError") {
        throw new ValidationError("Documents validation failed", {
          errors: error.errors.reduce((acc, err) => {
  ...
  async count(query = {}) {
    try {
      const queryFilter = normalizeMySQLFilter(query, Op);
