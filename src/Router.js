...
const { ipStrictLimiter, strictRateLimiter } = require("./middleware/rate-limit");
...
class Router {
  ...
  constructor(model, options = {}) {
    ...
    // If protected routes exists OR an always protected route exists in auth.routes
    // The JWT secret will be required to set
    const secret = options?.auth?.authMiddlewareOptions?.secret;
    if (!secret) {
      const protectedRoutes = options?.auth?.protectedRoutes;
      const hasAlwaysProtectedRoute = options?.auth?.routes?.some(auth_route_name => ROUTES.ALWAYS_PROTECTED_ROUTES.includes(auth_route_name));
      if (protectedRoutes === true || protectedRoutes?.length > 0 || hasAlwaysProtectedRoute) {
        throw new Error(
          `JWT secret is required for collection "${model?.modelName || "unknown"}" ` +
    ...
    if (this.auth) {
      ...
      this.loginSchema = schemas.auth.getLoginSchema(identifiantKey, passwordKey);
      this.registerSchema = schemas.auth.getRegisterSchema(identifiantKey, passwordKey, additionalFields);
    } else {
      this.loginSchema = schemas.auth.getLoginSchema();
      this.registerSchema = schemas.auth.getRegisterSchema();
    }
  ...
  getValidationMiddleware(routeName) {
    ...
    switch (routeName) {
      ...
      case "getAll":
        ...
        break;
      case "getOneById":
        middlewares.push(validate(schemas.crud.getOneById, "params"), validate(schemas.crud.getOneByIdQuery, "query"));
        break;
      case "search":
