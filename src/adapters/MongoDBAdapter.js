...
class MongoDBAdapter extends DatabaseAdapter {
  ...
  async connect() {
    try {
      // Enable strict query mode to prevent unexpected query behavior
      // and suppress deprecation warnings in Mongoose 8+
   ...
   * @returns {Promise<Object>} - Ping result from the MongoDB admin command
   */
  async ping() {
    ...
    return this.cache.getOrCompute(cacheKey, async () => {
      try {
        ...
        if (collectionOptions?.schema) {
          ...
          schema = new mongoose.Schema({}, { strict: false, strictQuery: false, collection: collectionName, versionKey: false });
        }
